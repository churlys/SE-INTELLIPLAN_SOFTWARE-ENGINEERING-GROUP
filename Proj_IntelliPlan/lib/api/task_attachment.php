...
function ensure_tasks_file_column(PDO $pdo): void {
    try {
        $existing = [];
        ...
        if (!isset($existing['file_id'])) {
            $pdo->exec('ALTER TABLE tasks ADD COLUMN file_id INT UNSIGNED NULL');
        }
    } catch (Throwable $e) {
        // Best-effort.
    }
...
$stmt = $pdo->prepare('UPDATE tasks SET file_id = ? WHERE id = ? AND user_id = ?');
$stmt->execute([$fileId, $taskId, $user['id']]);
